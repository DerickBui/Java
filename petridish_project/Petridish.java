...
public class Petridish {
  ...
  private void scanBact(int x, int y) {
    ...
    while (flag == true) {
      ...
      for (int i = (y + right); i > ((y - left) - 1); i--) { //backward scan
    	  if (wholeDish[x][i] == '*' || wholeDish[x][i] == '#') {
          	if (wholeDish[x][i] == '*') {
          		if (markit(x,i)) {
          			DLR = true;
    ...
    for (int i = x; i > -1; i--) {
      ...
      for (int j = (y + right); j > ((y - left) - 1); j--) {
    	  if (wholeDish[i][j] == '*' || wholeDish[i][j] == '#') {
            	if (wholeDish[i][j] == '*') {
            		markit(i,j);
            	}
    ...
    while (leftFlag == true) {
      if (((y - left) - 1) < 0) { //if left edge
        ...
        if (wholeDish[x][(y-left) - 1] == '*') { //if mark on left
        	boolean check = markit(x,((y-left) - 1));
        	if (check == true) {
        		flag = true;
            	left++;
        	}
        	...
        	if (wholeDish[x + 1][(y-left) - 1] == '*') {
        		boolean check = markit((x+1), ((y-left) - 1));
        		if (check == true) {
        			flag = true;
            		left++;
        		}
    ...
    while (rightFlag == true) {
      if (((y + right) + 1) > (wholeDish[x].length) - 1) { 
        ...
        if (wholeDish[x][(y + right) + 1] == '*') {
        	boolean check = markit(x,((y + right) + 1));
        	if (check == true) {
        		flag = true;
                right++;
        	}
        	...
        	if (wholeDish[x + 1][(y + right) + 1] == '*') {
        		boolean check = markit((x+1), ((y + right) + 1));
        		if (check == true) {
        			flag = true;
            		right++;
        		}
  ...
  // @param originY The y-axis of where the scan starts
  ...
  private boolean markit(int i, int j) {
